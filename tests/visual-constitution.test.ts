...
test('04 prompt is the primary home action',()=>['Che cosa vuoi costruire?','Descrivi un’applicazione o un sito','Costruisci'].forEach(value=>assert.match(page,new RegExp(value))));
test('05 app, website and prototype starts are explicit',()=>['Applicazione','Sito web','Prototipo'].forEach(value=>assert.match(page,new RegExp(value))));
...
test('09 project creation immediately starts the build',()=>{assert.match(page,/await runAutopilot\(data\.jobId,\s*data\.project\.id\)/);assert.match(page,/Avvio agenti/)});
