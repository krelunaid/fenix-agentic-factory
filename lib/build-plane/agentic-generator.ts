...
export type ProductBrief = {
  productName: string;
  summary: string;
  appType: 'crm' | 'commerce' | 'booking' | 'project' | 'content' | 'operations' | 'generic';
...
export function inferProductBrief(name: string, description: string, candidate?: unknown): ProductBrief {
  ...
  const requestedType = typeof raw.appType === 'string' && ['crm', 'commerce', 'booking', 'project', 'content', 'operations', 'generic'].includes(raw.appType) ? raw.appType as ProductBrief['appType'] : domain.type;
  return {
...
export function generateAgenticApplication(brief: ProductBrief): GeneratedFile[] {
